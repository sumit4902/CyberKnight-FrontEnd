...
export default function DigitalSignature() {
  ...
  const{user,token,logout}= AuthUser();
   ...
   {
    let url=`http://localhost:4202/api/v1/encrypt/Message?key=${key}&algoType=DigitalSignature`
    axios.post(url,
    {
      plainText:message
    },
   ...
   {  let url=`http://localhost:4202/api/v1/decrypt/Message?key=${key}&algoType=DigitalSignature`
      axios.post(url,
      {
        cipherText:message
      },
  ...
  return (
   <>
   <div className=" text-white">DigitalSignature</div>
   ...
   <div className=" flex ipadmini:flex-row flex-col gap-x-5 justify-between  ">
    <div className="  ipadmini:w-1/2 w-full">
       <textarea value={message} onChange={(event)=>{Setmessage(event.target.value)}} name="" id=""className=' w-full min-h-56 p-3 border-4  border-white placeholder-white font-semibold bg-transparent text-white rounded-lg focus:outline-none ' placeholder='Enter Message'></textarea>
    
       <input onChange={(event)=>{setKey(event.target.value)}} type="text" className=" border-2 border-white py-2 px-5 w-full placeholder-white font-semibold bg-transparent text-white rounded-lg focus:outline-none  " placeholder='After Getting Signature Check Message via Signature ' />
       <div className=" flex flex-row justify-evenly gap-y-4 py-3 ">
        <button onClick={()=>{handleencryption()}} className="border-2 py-2 px-8 rounded-md active:scale-[1.07]  text-white font-semibold  ">Get Signature</button>
        <button onClick={()=>{decryptionMessage()}} className="border-2 py-2 px-8 rounded-md active:scale-[1.07] text-white font-semibold">Verify Message</button>
       </div>
    </div>
    <div className=" ipadmini:w-1/2 w-full  border-white ">
        <div className='w-full min-h-56 border-4 border-white rounded-md p-2 text-white break-words'>{output}</div>
    </div>
   </div>
   {/* stepSection */}
