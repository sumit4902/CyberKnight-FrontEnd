...
export default function ShiftCipher() {
  ...
  const{token}= AuthUser();
   ...
   {
    let url=`http://localhost:4202/api/v1/encrypt/Message?key=${key}&algoType=ShiftCipher`
    axios.post(url,
    {
      plainText:message
    },
  ...
  return (
   <>
   <div className=" text-white">ShiftCipher</div>
   ...
   <div className=" flex ipadmini:flex-row flex-col gap-x-5 justify-between  ">
    <div className="  ipadmini:w-1/2 w-full">
       ...
       <input onChange={(event)=>{setKey(event.target.value)}} type="text" className=" border-2 border-white py-2 px-5 w-full placeholder-white font-semibold bg-transparent text-white rounded-lg focus:outline-none  " placeholder='No. of Shift ' />
