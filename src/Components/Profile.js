   ...
   function handleProfileApi()
    {
       ...
       {
          SetblogWriting({blogId:blogId,crimeType:crimeType,commitPlace:commitPlace,title:title,description:description,});
        ...
        {  
          ...
          //  console.log("Blog Update Successfully ..... "+response);
            Setblogmodel(false);
            window.location.reload();
          }).catch((error)=>{
           // console.log(error);
        ...
        //let confirmation = confirm("Sure You want Delete this Blog");
        // Delete Blogs  //
        function DeleteBlog()
        {  
                if(true){
          let url=`http://localhost:4202/api/v1/blog/delete/${blogWriting.blogId}`
          axios.delete(url,{
            headers:{
              "Authorization":`Bearer `+usertoken
          }
           ...
           <div className=" grid laptop:grid-cols-3 ipadmini:grid-cols-2 grid-cols-1 p-5 h-full min-h-[40rem] justify-items-center items-start  gap-y-7 laptop:px-20 ipadmini:px-10 ">
            {/* card -1 */}
            {
              profiledata.blogs.map((blog)=>{
               ...
               <button onClick={()=>{DeleteBlog()}} className=" py-1 px-3 border rounded-lg active:scale-[1.07] bg-purple-300 font-semibold ">Delete</button>
